...
const resolveChild = <T>(
	...
	setter: (value: T | T[], dynamic: boolean) => void,
...
const resolveStyle = (
	styles: Styles,
	resolved: Record<string, null | undefined | number | string> | string = {},
): Record<string, null | undefined | number | string> | string => {
	if (isString(styles)) {
...
const resolveArraysAndStatics = (() => {
	...
	const resolveArraysAndStaticsInner = (
		values: any[],
		resolved: any[],
		...
		for (let i = 0, l = values.length; i < l; i++) {
			...
			if (type === "string" || type === "number" || type === "bigint") {
				...
				hasObservables = resolveArraysAndStaticsInner(
					value,
					resolved,
					hasObservables,
				)[1];
