...
import { context, with as _with } from "../reactive";
import { SYMBOL_STORE_OBSERVABLE } from "../reactive";
...
const setAttribute = (
	element: HTMLElement,
	key: string,
	value: FunctionMaybe<null | undefined | boolean | number | string>,
): void => {
...
const setChildReplacementFunction = (
	parent: HTMLElement,
	fragment: Fragment,
	child: () => Child,
): void => {
...
const setChild = (
	parent: HTMLElement,
	child: Child,
	fragment: Fragment = FragmentUtils.make(),
): void => {
...
const setClass = (
	element: HTMLElement,
	key: string,
	value: FunctionMaybe<null | undefined | boolean>,
): void => {
	if (isFunction(value) && isFunctionReactive(value)) {
		useRenderEffect(() => {
...
const setClassBooleanStatic = (
	element: HTMLElement,
	value: boolean,
	key: null | undefined | boolean | string,
	keyPrev?: null | undefined | boolean | string,
): void => {
...
const setClassBoolean = (
	element: HTMLElement,
	value: boolean,
	key: FunctionMaybe<null | undefined | boolean | string>,
): void => {
			...
			if (isStore(object)) {
				for (let i = 0, l = object.length; i < l; i++) {
					const fn = untrack(() =>
						isFunction(object[i])
							? object[i]
							: object[SYMBOL_STORE_OBSERVABLE](String(i)),
					) as () => string | boolean | null | undefined; //TSC
				...
				for (const key in object) {
					const fn = untrack(() =>
						isFunction(object[key])
							? object[key]
							: (object as any)[SYMBOL_STORE_OBSERVABLE](key),
					) as () => boolean | null | undefined; //TSC
...
const setDirective = <T extends unknown[]>(
	element: HTMLElement,
	directive: string,
	args: T,
): void => {
...
const setEventStatic = (() => {
	...
	return (
		...
		if (event.startsWith("onmiddleclick")) {
			...
			value =
				_value && ((event: Event) => event["button"] === 1 && _value(event));
		}
...
const setRef = <T>(
	...
	useMicrotask(() =>
		untrack(() => values.forEach((value) => value?.(element))),
	);
};
...
const setStyleStatic = (() => {
	...
	const propertyNonDimensionalRe =
		/^(-|f[lo].*[^se]$|g.{5,}[^ps]$|z|o[pr]|(W.{5})?[lL]i.*(t|mp)$|an|(bo|s).{4}Im|sca|m.{6}[ds]|ta|c.*[st]$|wido|ini)/i;
			...
			for (const key in object) {
				const fn = untrack(() =>
					isFunction(object[key])
						? object[key]
						: (object as any)[SYMBOL_STORE_OBSERVABLE](key),
				) as () => number | string | null | undefined; //TSC
...
const setStyles = (
	...
	if (isFunction(object) || isArray(object)) {
		let objectPrev:
			...
			| undefined
			| string
			| Record<string, null | undefined | number | string>;
...
const setTemplateAccessor = (
	element: HTMLElement,
	key: string,
	value: TemplateActionProxy,
): void => {
	if (key === "children") {
