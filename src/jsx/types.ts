...
declare namespace JSX {
	...
	type DOMCSSProperties = {
		[key in keyof Omit<
			...
			| "getPropertyValue"
			| "getPropertyPriority"
		>]?: FunctionMaybe<Nullable<string | number>>;
	};
	...
	type Class = FunctionMaybe<
		Nullable<
			| string
			| ClassProperties
			| (FunctionMaybe<null | undefined | boolean | string> | Class)[]
		>
	>;
	...
	type Style = FunctionMaybe<
		Nullable<
			| string
			| StyleProperties
			| (FunctionMaybe<null | undefined | number | string> | Style)[]
		>
	>;

	type IntrinsicElement<T extends keyof IntrinsicElements> =
		IntrinsicElements[T];
	...
	> = Omit<TypedEvent, "currentTarget"> & { readonly currentTarget: T };
	type TargetedAnimationEvent<T extends EventTarget> = TargetedEvent<
		T,
		AnimationEvent
	>;
	type TargetedClipboardEvent<T extends EventTarget> = TargetedEvent<
		T,
		ClipboardEvent
	>;
	type TargetedCompositionEvent<T extends EventTarget> = TargetedEvent<
		T,
		CompositionEvent
	>;
	...
	type TargetedKeyboardEvent<T extends EventTarget> = TargetedEvent<
		T,
		KeyboardEvent
	>;
	type TargetedMouseEvent<T extends EventTarget> = TargetedEvent<T, MouseEvent>;
	type TargetedPointerEvent<T extends EventTarget> = TargetedEvent<
		T,
		PointerEvent
	>;
	type TargetedSubmitEvent<T extends EventTarget> = TargetedEvent<
		T,
		SubmitEvent
	>;
	type TargetedTouchEvent<T extends EventTarget> = TargetedEvent<T, TouchEvent>;
	type TargetedTransitionEvent<T extends EventTarget> = TargetedEvent<
		T,
		TransitionEvent
	>;
	...
	type AnimationEventHandler<T extends EventTarget> = EventHandler<
		TargetedAnimationEvent<T>
	>;
	type ClipboardEventHandler<T extends EventTarget> = EventHandler<
		TargetedClipboardEvent<T>
	>;
	type CompositionEventHandler<T extends EventTarget> = EventHandler<
		TargetedCompositionEvent<T>
	>;
	type DragEventHandler<T extends EventTarget> = EventHandler<
		TargetedDragEvent<T>
	>;
	type FocusEventHandler<T extends EventTarget> = EventHandler<
		TargetedFocusEvent<T>
	>;
	type GenericEventHandler<T extends EventTarget> = EventHandler<
		TargetedEvent<T>
	>;
	type InputEventHandler<T extends EventTarget> = EventHandler<
		TargetedInputEvent<T>
	>;
	type KeyboardEventHandler<T extends EventTarget> = EventHandler<
		TargetedKeyboardEvent<T>
	>;
	type MouseEventHandler<T extends EventTarget> = EventHandler<
		TargetedMouseEvent<T>
	>;
	type PointerEventHandler<T extends EventTarget> = EventHandler<
		TargetedPointerEvent<T>
	>;
	type SubmitEventHandler<T extends EventTarget> = EventHandler<
		TargetedSubmitEvent<T>
	>;
	type TouchEventHandler<T extends EventTarget> = EventHandler<
		TargetedTouchEvent<T>
	>;
	type TransitionEventHandler<T extends EventTarget> = EventHandler<
		TargetedTransitionEvent<T>
	>;
	type UIEventHandler<T extends EventTarget> = EventHandler<TargetedUIEvent<T>>;
	type WheelEventHandler<T extends EventTarget> = EventHandler<
		TargetedWheelEvent<T>
	>;
	...
	interface AriaAttributes {
		...
		ariaAutoComplete?: FunctionMaybe<
			Nullable<"none" | "inline" | "list" | "both">
		>;
		...
		ariaDropEffect?: FunctionMaybe<
			Nullable<"none" | "copy" | "execute" | "link" | "move" | "popup">
		>;
		...
		ariaInvalid?: FunctionMaybe<
			Nullable<boolean | "false" | "true" | "grammar" | "spelling">
		>;
		...
		ariaSort?: FunctionMaybe<
			Nullable<"none" | "ascending" | "descending" | "other">
		>;
	...
	interface Directives {
		...
		__converged_internal1__: [];
		__converged_internal2__: [];
		// name: [arg1: unknown, arg2: unknown, ...argN: unknown]
	}

	type DirectiveAttributes = {
		[Directive in keyof Directives as `use:${Directive}`]?: Directives[Directive] extends [
			infer U,
		]
			? U | [U]
	...
	interface EventAttributes<T extends EventTarget> {
		...
		onCompositionEndCapture?: ObservableMaybe<
			Nullable<CompositionEventHandler<T>>
		>;
		onCompositionStart?: ObservableMaybe<Nullable<CompositionEventHandler<T>>>;
		onCompositionStartCapture?: ObservableMaybe<
			Nullable<CompositionEventHandler<T>>
		>;
		onCompositionUpdate?: ObservableMaybe<Nullable<CompositionEventHandler<T>>>;
		onCompositionUpdateCapture?: ObservableMaybe<
			Nullable<CompositionEventHandler<T>>
		>;
		/* DETAILS EVENTS */
		...
		onGotPointerCaptureCapture?: ObservableMaybe<
			Nullable<PointerEventHandler<T>>
		>;
		onLostPointerCapture?: ObservableMaybe<Nullable<PointerEventHandler<T>>>;
		onLostPointerCaptureCapture?: ObservableMaybe<
			Nullable<PointerEventHandler<T>>
		>;
		/* UI EVENTS */
		...
		onAnimationStartCapture?: ObservableMaybe<
			Nullable<AnimationEventHandler<T>>
		>;
		...
		onAnimationIterationCapture?: ObservableMaybe<
			Nullable<AnimationEventHandler<T>>
		>;
		/* TRANSITION EVENTS */
		onTransitionEnd?: ObservableMaybe<Nullable<TransitionEventHandler<T>>>;
		onTransitionEndCapture?: ObservableMaybe<
			Nullable<TransitionEventHandler<T>>
		>;
	}
	...
	interface VoidHTMLAttributes<T extends EventTarget>
		...
		contentEditable?: FunctionMaybe<
			Nullable<
				boolean | "caret" | "events" | "plaintext-only" | "typing" | "inherit"
			>
		>;
		...
		autoCapitalize?: FunctionMaybe<
			Nullable<"off" | "none" | "on" | "sentences" | "words" | "characters">
		>;
	...
	interface SVGAttributes<T extends EventTarget = SVGElement>
		...
		alignmentBaseline?: FunctionMaybe<
			Nullable<
				| "auto"
				| "baseline"
				| "before-edge"
		...
		arabicForm?: FunctionMaybe<
			Nullable<"initial" | "medial" | "terminal" | "isolated">
		>;
		...
		colorInterpolationFilters?: FunctionMaybe<
			Nullable<"auto" | "sRGB" | "linearRGB" | "inherit">
		>;
		...
		strokeLinecap?: FunctionMaybe<
			Nullable<"butt" | "round" | "square" | "inherit">
		>;
		strokeLinejoin?: FunctionMaybe<
			Nullable<"miter" | "round" | "bevel" | "inherit">
		>;
	...
	interface BrHTMLAttributes<T extends EventTarget>
		extends VoidHTMLAttributes<T> {}
	...
	interface ColHTMLAttributes<T extends EventTarget>
		extends VoidHTMLAttributes<T> {
		span?: FunctionMaybe<Nullable<number>>;
		width?: FunctionMaybe<Nullable<number | string>>;
	}
	...
	interface HrHTMLAttributes<T extends EventTarget>
		extends VoidHTMLAttributes<T> {}
	...
	interface ImgHTMLAttributes<T extends EventTarget>
		extends VoidHTMLAttributes<T> {
		alt?: FunctionMaybe<Nullable<string>>;
	...
	interface TdHTMLAttributes<T extends EventTarget> extends HTMLAttributes<T> {
		align?: FunctionMaybe<
			Nullable<"left" | "center" | "right" | "justify" | "char">
		>;
		colSpan?: FunctionMaybe<Nullable<number>>;
		headers?: FunctionMaybe<Nullable<string>>;
		rowSpan?: FunctionMaybe<Nullable<number>>;
	...
	interface WbrHTMLAttributes<T extends EventTarget>
		extends VoidHTMLAttributes<T> {}
