...
const template = <P = {}>(
	...
	const checkValidProperty = (property: unknown): property is string => {
		...
		throw new Error(
			`Invalid property, only alphanumeric properties are allowed inside templates, received: "${property}"`,
		);
	};
	...
	const makeActionsWithNodesAndTemplate = (): {
		actionsWithNodes: TemplateActionWithNodes[];
		root: Element;
	} => {
		...
		throw new Error(
			"Invalid template, it must return a function that returns an Element",
		);
	};

	const makeActionsWithPaths = (
		actionsWithNodes: TemplateActionWithNodes[],
	): TemplateActionWithPaths[] => {
	...
	const makeNodePath = (() => {
		...
		return (node: Node): TemplateActionPath => {
			...
			while (parent) {
				const index = !child.previousSibling
					? 0
					: !child.nextSibling
					  ? -0
					  : indexOf(parent.childNodes, child);
	...
	const makeReviverPaths = (
		actionsWithPaths: TemplateActionWithPaths[],
	): TemplateActionPath[] => {
	...
	const makeReviverVariablesData = (
		paths: TemplateActionPath[],
	...
	const makeReviverVariables = (
		actionsWithPaths: TemplateActionWithPaths[],
	): { assignments: string[]; map: Map<TemplateActionPath, string> } => {
		//TODO: Optimize this further, there's some duplication and unnecessary work being done
	...
	const makeReviverActions = (
		actionsWithPaths: TemplateActionWithPaths[],
		variables: Map<TemplateActionPath, string>,
	): string[] => {
		const actions: string[] = [];

		for (let i = 0, l = actionsWithPaths.length; i < l; i++) {
			//TODO: Write this more cleanly, with a single case

			const [nodePath, method, prop, key, targetNodePath] = actionsWithPaths[i];

			if (targetNodePath) {
				actions.push(
					`this.${method} ( props["${prop}"], ${variables.get(
						targetNodePath,
					)} );`,
				);
			} else if (key) {
				actions.push(
					`this.${method} ( ${variables.get(
						nodePath,
					)}, "${key}", props["${prop}"] );`,
				);
			} else {
				actions.push(
					`this.${method} ( ${variables.get(nodePath)}, props["${prop}"] );`,
				);
			}
		}
